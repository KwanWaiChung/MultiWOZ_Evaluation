...
from mwzeval.utils import (
    has_domain_predictions,
    get_domain_estimates_from_state,
)
...
class Evaluator:
    ...
    def evaluate(self, input_data, detail: bool = False):
        ...
        if detail:
            if self.success:
                ...
                failure = success.pop("failure")
            return {
                "bleu": get_bleu(input_data, self.reference_dialogs)
                if self.bleu
                else None,
                "success": success if self.success else None,
                "failure": failure if self.success else None,
                "richness": get_richness(input_data)
                if self.richness
                else None,
                "dst": get_dst(input_data, self.gold_states)
                if self.dst
                else None,
            }
...
def get_richness(input_data):
    ...
    cond = [
        unique_grams[1][(h, w)] / unique_grams[0][h]
        for h, w in unique_grams[1]
    ]
...
def get_success(input_data, database, goals, booked_domains, detail):
    ...
    for dialog_id, dialog in input_data.items():
        ...
        (
            match,
            success,
            provided_requestable_slots,
            requestable_slots_in_goal,
        ) = get_dialog_success(
            goals[dialog_id],
        ...
        if not success or not success["total"]:
            failure[dialog_id] = {
                "responses": utterances,
                "reqt_in_goal": requestable_slots_in_goal,
                "provided_slots": provided_requestable_slots,
            }

        for domain in set(match) | set(success):
            total[domain] += 1
            match_rate[domain] = match.get(domain, 0) + match_rate.get(
                domain, 0
            )
            success_rate[domain] = success.get(domain, 0) + success_rate.get(
                domain, 0
            )

    match_rate = {
        k: round(100 * match_rate[k] / total[k], 1) for k in match_rate
    }
    success_rate = {
        k: round(100 * success_rate[k] / total[k], 1) for k in success_rate
    }
    if detail:
        return {
            "inform": match_rate,
                ...
                if requestable_slot in system_utterance:
                    ...
                    if requestable_slot == "REFERENCE":
                        if current_domain in booked_domain:
                            provided_requestable_slots[current_domain].add(
                                "REFERENCE"
                            )
                    else:
                        provided_requestable_slots[current_domain].add(
    ...
    if match["total"]:
        for domain in goal:
            ...
            provided_and_wanted_slots = (
                provided_requestable_slots[domain]
...
def get_dst(input_data, reference_states, fuzzy_ratio=95):
    ...
    def compare(hyp, ref):
        ...
        for slot, value in hyp.items():
            if (
                slot in ref
                and fuzz.partial_ratio(value, ref[slot]) > fuzzy_ratio
            ):
                tp += 1
            else:
                fp += 1
        for slot, value in ref.items():
            if (
                slot not in hyp
                or fuzz.partial_ratio(hyp[slot], value) <= fuzzy_ratio
            ):
                fn += 1
        return tp, fp, fn
