...
def normalize_data(input_data):
    ...
    for dial_id, dialogue in input_data.items():
        for turn in dialogue:
            ...
            turn["response"] = md.detokenize(
                mt.tokenize(
                    turn["response"].replace("-s", "").replace("-ly", "")
                )
            )
...
def normalize_slot_name(slot_name):
    ...
    reverse_slot_name_mapping = {
        s: k for k, v in slot_name_mapping.items() for s in v
    }
    if slot_name not in reverse_slot_name_mapping:
        print(
            f"Unknown slot name: {slot_name}. Please use another slot names or"
            " customize the slot mapping!"
        )
        return ""
