...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--detail",
        action="store_true",
        help="Provide failure results.",
    )
    args = parser.parse_args()

    if not args.bleu and not args.success and not args.richness:
        sys.stderr.write(
            "error: Missing argument, at least one of -b, -s, and -r must be"
            " used!\n"
        )
        parser.print_help()
        sys.exit(1)
    ...
    results = e.evaluate(input_data, detail=args.detail)

    for metric, values in results.items():
        if metric.lower() == "failure":
            continue
